...
consistent with the visible form. Unseen façades may continue observed opening
patterns, materials and surface articulation where appropriate; being unseen is
not by itself a reason to leave them blank. These details remain explicitly assumed,
and do not justify unsupported major additions or distinctive features.
...
- Exterior completion guidance now permits plausible continuation of observed
  opening patterns and surface treatment on unseen façades. Inferred detail stays
  subordinate to visible evidence and is documented as assumed.
- This addresses test 18's deliberate blank side/rear façades. It changes the
  modelling guidance, not the MCP tools, scoring, or stopping policy; no additional
  renders or checks are required. The next reconstruction tests will assess the
  behavioral effect.
